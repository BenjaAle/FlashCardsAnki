...
chat_ia = client.chats.create(
    ...
    config={
        "system_instruction": "Eres un amigable y experto tutor de inglés. Responde de forma concisa, conversacional y educativa. Usa ejemplos claros."
    },
)
...
def extraer_anki():
    ...
    # 🌟 EL NUEVO PROMPT: Ahora incluye categorías
    prompt = f"""
    ...
    REGLA 3: El campo "categoria" DEBE ser ESTRICTAMENTE una de las siguientes opciones (cópiala tal cual):
    - Vocabulario
    - Phrasal Verbs
    - Falsos Amigos
    - Verbos Irregulares
    ...
    try:
        ...
        for i, carta in enumerate(lista_cartas):
            ...
            # 🌟 NUEVA LÓGICA DE SUBMAZOS
            categoria_elegida = str(carta.get("categoria", "Vocabulario")).strip()
            # Si Gemini inventa una categoría rara, la forzamos a Vocabulario
            categorias_validas = [
                "Vocabulario",
                "Phrasal Verbs",
                "Falsos Amigos",
            ...
            if categoria_elegida not in categorias_validas:
                categoria_elegida = "Vocabulario"

            # Creamos la ruta del mazo: Ej. "Ingles_IA::Phrasal Verbs"
            mazo_destino = f"{NOMBRE_MAZO}::{categoria_elegida}"

            # Le pedimos a Anki que cree el submazo si no existe
            try:
            ...
            # 4. INYECTAR A ANKI (AHORA AL SUBMAZO CORRESPONDIENTE)
            try:
                invoke_anki(
                    "addNote",
                    note={
